...
// CHtmlWindow. CHtmlWindow runs HTML markup through all registered
// processors before displaying it, thus allowing for on-the-fly
// modifications of the markup.

class WXDLLIMPEXP_HTML CHtmlProcessor : public wxObject
{
    DECLARE_ABSTRACT_CLASS(CHtmlProcessor)

public:
    CHtmlProcessor() : wxObject(), m_enabled(true) {}
    virtual ~CHtmlProcessor() {}
    ...
    // Enable/disable the processor. CHtmlWindow won't use a disabled
