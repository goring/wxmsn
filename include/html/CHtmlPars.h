...
// RCS-ID:      $Id: CHtmlPars.h,v 1.22 2004/09/27 19:06:39 ABX Exp $
...
class WXDLLIMPEXP_HTML CHtmlParser;
class WXDLLIMPEXP_HTML CHtmlTagHandler;
class WXDLLIMPEXP_HTML CHtmlEntitiesParser;

class CHtmlTextPieces;
...
// 2 tags.
class WXDLLIMPEXP_HTML CHtmlParser : public wxObject
{
    DECLARE_ABSTRACT_CLASS(CHtmlParser)
...
public:
    ...
    CHtmlTag *GetCurrentTag() const { return m_CurTag; }
    ...
    // (see CHtmlWinParser for details).
    virtual wxObject* GetProduct() = 0;

    // adds handler to the list & hash table of handlers.
    virtual void AddTagHandler(CHtmlTagHandler *handler);
    ...
    void PushTagHandler(CHtmlTagHandler *handler, wxString tags);
    ...
    CHtmlEntitiesParser *GetEntitiesParser() const { return m_entitiesParser; }

protected:
    // DOM structure
    void CreateDOMTree();
    void DestroyDOMTree();
    void CreateDOMSubTree(CHtmlTag *cur,
    ...
    virtual void AddTag(const CHtmlTag& tag);
...
protected:
    ...
    CHtmlParserState *m_SavedStates;
    ...
    // (see CHtmlWinParser for details about filling it)
    ...
    DECLARE_NO_COPY_CLASS(CHtmlParser)
    ...
    // entity parse
    CHtmlEntitiesParser *m_entitiesParser;
...
// This class (and derived classes) cooperates with CHtmlParser.
...
class WXDLLIMPEXP_HTML CHtmlTagHandler : public wxObject
{
    DECLARE_ABSTRACT_CLASS(CHtmlTagHandler)

public:
    CHtmlTagHandler() : wxObject () { m_Parser = NULL; }

    // Sets the parser.
    // NOTE : each _instance_ of handler is guaranteed to be called
    // only by one parser. This means you don't have to care about
    // reentrancy.
    virtual void SetParser(CHtmlParser *parser)
        { m_Parser = parser; }
    ...
    //                  false etherwise
    virtual bool HandleTag(const CHtmlTag& tag) = 0;

protected:
    // parses input between beginning and ending tag.
    // m_Parser must be set.
    void ParseInner(const CHtmlTag& tag)
        { m_Parser->DoParsing(tag.GetBeginPos(), tag.GetEndPos1()); }

    CHtmlParser *m_Parser;

    DECLARE_NO_COPY_CLASS(CHtmlTagHandler)
};
...
class WXDLLIMPEXP_HTML CHtmlEntitiesParser : public wxObject
{
    DECLARE_DYNAMIC_CLASS(CHtmlEntitiesParser)

public:
    CHtmlEntitiesParser();
    virtual ~CHtmlEntitiesParser();
...
protected:
    ...
    DECLARE_NO_COPY_CLASS(CHtmlEntitiesParser)
};
