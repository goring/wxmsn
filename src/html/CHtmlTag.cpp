...
{
    // this is "pos" value passed to CHtmlTag's constructor.
...
IMPLEMENT_CLASS(CHtmlTagsCache,wxObject)
...
CHtmlTagsCache::CHtmlTagsCache(const wxString& source)
{
    ...
    while (pos < lng)
    {
        if (src[pos] == wxT('<'))   // tag found:
        {
            if (m_CacheSize % CACHE_INCREMENT == 0)
                m_Cache = (CHtmlCacheItem*) realloc(m_Cache, (m_CacheSize + CACHE_INCREMENT) * sizeof(CHtmlCacheItem));
            int tg = m_CacheSize++;
            int stpos = pos++;
...
void CHtmlTagsCache::QueryTag(int at, int* end1, int* end2)
{
...
IMPLEMENT_CLASS(CHtmlTag,wxObject)

CHtmlTag::CHtmlTag(CHtmlTag *parent,
                     const wxString& source, int pos, int end_pos,
                     CHtmlTagsCache *cache,
                     CHtmlEntitiesParser *entParser) : wxObject()
{
    /* Setup DOM relations */
...
bool CHtmlTag::HasParam(const wxString& par) const
{
    return (m_ParamNames.Index(par, false) != wxNOT_FOUND);
}

wxString CHtmlTag::GetParam(const wxString& par, bool with_commas) const
{
...
int CHtmlTag::ScanParam(const wxString& par,
                         const wxChar *format,
                         void *param) const
{
...
bool CHtmlTag::GetParamAsColour(const wxString& par, wxColour *clr) const
{
...
bool CHtmlTag::GetParamAsInt(const wxString& par, int *clr) const
{
...
wxString CHtmlTag::GetAllParams() const
{
...
CHtmlTag *CHtmlTag::GetFirstSibling() const
{
    if (m_Parent)
        return m_Parent->m_FirstChild;
    else
    {
        CHtmlTag *cur = (CHtmlTag*)this;
        while (cur->m_Prev)
            cur = cur->m_Prev;
        return cur;
    }
...
CHtmlTag *CHtmlTag::GetLastSibling() const
{
    if (m_Parent)
