...
CHtmlDCRenderer::CHtmlDCRenderer() : wxObject()
{
    m_DC = NULL;
    m_Width = m_Height = 0;
    m_Cells = NULL;
    m_Parser = new CHtmlWinParser(NULL);
...
CHtmlDCRenderer::~CHtmlDCRenderer()
{
...
void CHtmlDCRenderer::SetDC(wxDC *dc, double pixel_scale)
{
    m_DC = dc;
...
void CHtmlDCRenderer::SetSize(int width, int height)
{
    m_Width = width;
    m_Height = height;
}
...
void CHtmlDCRenderer::SetHtmlText(const wxString& html, const wxString& basepath, bool isdir)
{
    if (m_DC == NULL) return;
    ...
    m_Cells = (CHtmlContainerCell*) m_Parser->Parse(html);
    m_Cells->SetIndent(0, wxHTML_INDENT_ALL, wxHTML_UNITS_PIXELS);
    m_Cells->Layout(m_Width, 0);
}


void CHtmlDCRenderer::SetFonts(wxString normal_face, wxString fixed_face,
                                const int *sizes)
{
...
void CHtmlDCRenderer::SetStandardFonts(int size,
...
int CHtmlDCRenderer::Render(int x, int y, int from, int dont_render,
                             int maxHeight,
...
int CHtmlDCRenderer::GetTotalHeight()
{
...
wxList CHtmlPrintout::m_Filters;

CHtmlPrintout::CHtmlPrintout(const wxString& title) : wxPrintout(title)
{
    m_Renderer = new CHtmlDCRenderer;
    m_RendererHdr = new CHtmlDCRenderer;
...
void CHtmlPrintout::CleanUpStatics()
{
    WX_CLEAR_LIST(wxList, m_Filters);
}

// Adds input filter
void CHtmlPrintout::AddFilter(CHtmlFilter *filter)
{
    m_Filters.Append(filter);
}

void CHtmlPrintout::OnPreparePrinting()
{
...
bool CHtmlPrintout::OnBeginDocument(int startPage, int endPage)
{
...
bool CHtmlPrintout::OnPrintPage(int page)
{
    wxDC *dc = GetDC();
    if (dc)
    {
...
void CHtmlPrintout::GetPageInfo(int *minPage, int *maxPage, int *selPageFrom, int *selPageTo)
{
    *minPage = 1;
    *maxPage = m_NumPages;
    *selPageFrom = 1;
...
bool CHtmlPrintout::HasPage(int pageNum)
{
...
void CHtmlPrintout::SetHtmlText(const wxString& html, const wxString &basepath, bool isdir)
{
    m_Document = html;
    m_BasePath = basepath;
...
void CHtmlPrintout::SetHtmlFile(const wxString& htmlfile)
{
    wxFileSystem fs;
    wxFSFile *ff;
...
void CHtmlPrintout::SetHeader(const wxString& header, int pg)
{
...
void CHtmlPrintout::SetFooter(const wxString& footer, int pg)
{
    if (pg == wxPAGE_ALL || pg == wxPAGE_EVEN)
        m_Footers[0] = footer;
    if (pg == wxPAGE_ALL || pg == wxPAGE_ODD)
...
void CHtmlPrintout::RenderPage(wxDC *dc, int page)
{
    wxBusyCursor wait;
...
wxString CHtmlPrintout::TranslateHeader(const wxString& instr, int page)
{
    wxString r = instr;
    wxString num;
...
void CHtmlPrintout::SetMargins(float top, float bottom, float left, float right, float spaces)
{
    m_MarginTop = top;
...
void CHtmlPrintout::SetFonts(wxString normal_face, wxString fixed_face,
                              const int *sizes)
{
...
void CHtmlPrintout::SetStandardFonts(int size,
                                      const wxString& normal_face,
                                      const wxString& fixed_face)
{
...
CHtmlEasyPrinting::CHtmlEasyPrinting(const wxString& name, wxWindow *parentWindow)
{
...
CHtmlEasyPrinting::~CHtmlEasyPrinting()
{
    delete m_PrintData;
...
wxPrintData *CHtmlEasyPrinting::GetPrintData()
{
    if (m_PrintData == NULL)
...
bool CHtmlEasyPrinting::PreviewFile(const wxString &htmlfile)
{
    CHtmlPrintout *p1 = CreatePrintout();
    p1->SetHtmlFile(htmlfile);
    CHtmlPrintout *p2 = CreatePrintout();
...
bool CHtmlEasyPrinting::PreviewText(const wxString &htmltext, const wxString &basepath)
{
    CHtmlPrintout *p1 = CreatePrintout();
...
bool CHtmlEasyPrinting::PrintFile(const wxString &htmlfile)
{
    CHtmlPrintout *p = CreatePrintout();
...
bool CHtmlEasyPrinting::PrintText(const wxString &htmltext, const wxString &basepath)
{
    CHtmlPrintout *p = CreatePrintout();
...
bool CHtmlEasyPrinting::DoPreview(CHtmlPrintout *printout1, CHtmlPrintout *printout2)
{
...
bool CHtmlEasyPrinting::DoPrint(CHtmlPrintout *printout)
{
    wxPrintDialogData printDialogData(*GetPrintData());
...
void CHtmlEasyPrinting::PageSetup()
{
    if (!GetPrintData()->Ok())
    {
...
void CHtmlEasyPrinting::SetHeader(const wxString& header, int pg)
{
    if (pg == wxPAGE_ALL || pg == wxPAGE_EVEN)
        m_Headers[0] = header;
    if (pg == wxPAGE_ALL || pg == wxPAGE_ODD)
...
void CHtmlEasyPrinting::SetFooter(const wxString& footer, int pg)
{
    if (pg == wxPAGE_ALL || pg == wxPAGE_EVEN)
        m_Footers[0] = footer;
    if (pg == wxPAGE_ALL || pg == wxPAGE_ODD)
...
void CHtmlEasyPrinting::SetFonts(wxString normal_face, wxString fixed_face,
                                  const int *sizes)
{
...
void CHtmlEasyPrinting::SetStandardFonts(int size,
                                          const wxString& normal_face,
                                          const wxString& fixed_face)
{
...
CHtmlPrintout *CHtmlEasyPrinting::CreatePrintout()
{
    CHtmlPrintout *p = new CHtmlPrintout(m_Name);
...
class CHtmlPrintingModule: public wxModule
{
DECLARE_DYNAMIC_CLASS(CHtmlPrintingModule)
public:
    CHtmlPrintingModule() : wxModule() {}
    bool OnInit() { return true; }
    void OnExit() { CHtmlPrintout::CleanUpStatics(); }
};

IMPLEMENT_DYNAMIC_CLASS(CHtmlPrintingModule, wxModule)
