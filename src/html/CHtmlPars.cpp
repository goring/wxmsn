/////////////////////////////////////////////////////////////////////////////
// Name:        htmlpars.cpp
// Purpose:     CHtmlParser class (generic parser)
...
#pragma implementation "CHtmlPars.h"
#endif
...
class CHtmlTextPiece
{
public:
    CHtmlTextPiece(int pos, int lng) : m_pos(pos), m_lng(lng) {}
    int m_pos, m_lng;
};

WX_DECLARE_OBJARRAY(CHtmlTextPiece, CHtmlTextPieces);
WX_DEFINE_OBJARRAY(CHtmlTextPieces);
...
public:
    CHtmlTag         *m_curTag;
    CHtmlTag         *m_tags;
    CHtmlTextPieces  *m_textPieces;
    int                m_curTextPiece;
    wxString           m_source;
    CHtmlParserState *m_nextState;
};
...
wxObject* CHtmlParser::Parse(const wxString& source)
{
    InitParser(source);
    DoParsing();
...
void CHtmlParser::InitParser(const wxString& source)
{
    SetSource(source);
    m_stopParsing = false;
}

void CHtmlParser::DoneParser()
{
    DestroyDOMTree();
}

void CHtmlParser::SetSource(const wxString& src)
{
    DestroyDOMTree();
    m_Source = src;
    CreateDOMTree();
...
void CHtmlParser::CreateDOMTree()
{
    CHtmlTagsCache cache(m_Source);
    m_TextPieces = new CHtmlTextPieces;
...
void CHtmlParser::CreateDOMSubTree(CHtmlTag *cur,
                                    ...
                                    CHtmlTagsCache *cache)
{
    ...
    while (i < end_pos)
    {
        ...
        if (c == wxT('<'))
        {
            ...
            if (i - textBeginning > 0)
                m_TextPieces->Add(
                    CHtmlTextPiece(textBeginning, i - textBeginning));
            ...
            else if (i < end_pos-1 && m_Source.GetChar(i+1) != wxT('/'))
            {
                CHtmlTag *chd;
                if (cur)
                    chd = new CHtmlTag(cur, m_Source,
                                        i, end_pos, cache, m_entitiesParser);
                else
                {
                    chd = new CHtmlTag(NULL, m_Source,
                                        i, end_pos, cache, m_entitiesParser);
                    if (!m_Tags)
                    {
    ...
    if (end_pos - textBeginning > 0)
        m_TextPieces->Add(
            CHtmlTextPiece(textBeginning, end_pos - textBeginning));
}

void CHtmlParser::DestroyDOMTree()
{
    CHtmlTag *t1, *t2;
    t1 = m_Tags;
    while (t1)
...
void CHtmlParser::DoParsing(int begin_pos, int end_pos)
{
    if (end_pos <= begin_pos) return;

    CHtmlTextPieces& pieces = *m_TextPieces;
...
void CHtmlParser::AddTag(const CHtmlTag& tag)
{
    CHtmlTagHandler *h;
    bool inner = false;

    h = (CHtmlTagHandler*) m_HandlersHash.Get(tag.GetName());
    if (h)
    {
...
void CHtmlParser::AddTagHandler(CHtmlTagHandler *handler)
{
...
void CHtmlParser::PushTagHandler(CHtmlTagHandler *handler, wxString tags)
{
...
void CHtmlParser::PopTagHandler()
{
...
void CHtmlParser::SetSourceAndSaveState(const wxString& src)
{
    CHtmlParserState *s = new CHtmlParserState;
...
bool CHtmlParser::RestoreState()
{
    if (!m_SavedStates) return false;

    DestroyDOMTree();

    CHtmlParserState *s = m_SavedStates;
...
IMPLEMENT_ABSTRACT_CLASS(CHtmlTagHandler,wxObject)
...
IMPLEMENT_DYNAMIC_CLASS(CHtmlEntitiesParser,wxObject)

CHtmlEntitiesParser::CHtmlEntitiesParser()
...
void CHtmlEntitiesParser::SetEncoding(wxFontEncoding encoding)
{
#if wxUSE_WCHAR_T && !wxUSE_UNICODE
...
wxString CHtmlEntitiesParser::Parse(const wxString& input)
{
    const wxChar *c, *last;
...
struct CHtmlEntityInfo
{
    const wxChar *name;
    unsigned code;
};

extern "C" int LINKAGEMODE CHtmlEntityCompare(const void *key, const void *item)
{
    return wxStrcmp((wxChar*)key, ((CHtmlEntityInfo*)item)->name);
}

#if !wxUSE_UNICODE
wxChar CHtmlEntitiesParser::GetCharForCode(unsigned code)
{
#if wxUSE_WCHAR_T
    char buf[2];
    wchar_t wbuf[2];
...
wxChar CHtmlEntitiesParser::GetEntityChar(const wxString& entity)
{
    unsigned code = 0;
    ...
    {
        static CHtmlEntityInfo substitutions[] = {
            { wxT("AElig"),198 },
            { wxT("Aacute"),193 },
            { wxT("Acirc"),194 },
            { wxT("Agrave"),192 },
        ...
        CHtmlEntityInfo *info;
        info = (CHtmlEntityInfo*) bsearch(entity.c_str(), substitutions,
                                           substitutions_cnt,
                                           sizeof(CHtmlEntityInfo),
                                           CHtmlEntityCompare);
        if (info)
            code = info->code;
    }
...
wxFSFile *CHtmlParser::OpenURL(CHtmlURLType WXUNUSED(type),
...
//-----------------------------------------------------------------------------
// CHtmlParser::ExtractCharsetInformation
//-----------------------------------------------------------------------------

class wxMetaTagParser : public CHtmlParser
{
public:
    wxMetaTagParser() { }
...
class wxMetaTagHandler : public CHtmlTagHandler
{
public:
    wxMetaTagHandler(wxString *retval) : CHtmlTagHandler(), m_retval(retval) {}
    wxString GetSupportedTags() { return wxT("META,BODY"); }
    bool HandleTag(const CHtmlTag& tag);
...
bool wxMetaTagHandler::HandleTag(const CHtmlTag& tag)
{
...
/*static*/
wxString CHtmlParser::ExtractCharsetInformation(const wxString& markup)
{
    wxString charset;
