/////////////////////////////////////////////////////////////////////////////
// Name:        m_tables.cpp
// Purpose:     CHtml module for tables
...
class CHtmlTableCell : public CHtmlContainerCell
{
protected:
...
public:
    CHtmlTableCell(CHtmlContainerCell *parent, const CHtmlTag& tag, double pixel_scale = 1.0);
    ~CHtmlTableCell();
    virtual void Layout(int w, int vw);

    void AddRow(const CHtmlTag& tag);
    void AddCell(CHtmlContainerCell *cell, const CHtmlTag& tag);
...
private:
    ...
    DECLARE_NO_COPY_CLASS(CHtmlTableCell)
};



CHtmlTableCell::CHtmlTableCell(CHtmlContainerCell *parent, const CHtmlTag& tag, double pixel_scale)
 : CHtmlContainerCell(parent)
{
...
void CHtmlTableCell::ReallocCols(int cols)
{
    int i,j;
...
void CHtmlTableCell::ReallocRows(int rows)
{
...
void CHtmlTableCell::AddRow(const CHtmlTag& tag)
{
    m_ActualCol = -1;
...
void CHtmlTableCell::AddCell(CHtmlContainerCell *cell, const CHtmlTag& tag)
{
...
void CHtmlTableCell::ComputeMinMaxWidths()
{
    ...
    for (int c = 0; c < m_NumCols; c++)
    {
        for (int r = 0; r < m_NumRows; r++)
        {
            ...
            {
                cell.cont->Layout(2*m_Padding + 1, 0);
...
void CHtmlTableCell::Layout(int w, int vw)
{
    ComputeMinMaxWidths();
...
TAG_HANDLER_BEGIN(TABLE, "TABLE,TR,TD,TH")
    ...
    TAG_HANDLER_PROC(tag)
    {
        ...
        {
            CHtmlTableCell *oldt = m_Table;
            CHtmlContainerCell *oldcont;

            oldcont = c = m_WParser->OpenContainer();

            m_Table = new CHtmlTableCell(c, tag);
        ...
        else if (m_Table)
        {
            ...
            {
                c = m_WParser->SetContainer(new CHtmlContainerCell(m_Table));
