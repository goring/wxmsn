/////////////////////////////////////////////////////////////////////////////
// Name:        m_image.cpp
// Purpose:     CHtml module for displaying images
// Author:      Vaclav Slavik
// RCS-ID:      $Id: m_image.cpp,v 1.53 2005/07/26 23:33:27 VZ Exp $
...
// ---------------------------------------------------------------------------
// CHtmlImageMapAreaCell
//                  0-width, 0-height cell that represents single area in
//                  imagemap (it's GetLink is called from CHtmlImageCell's)
// ---------------------------------------------------------------------------

class CHtmlImageMapAreaCell : public CHtmlCell
{
    public:
    ...
    protected:
        ...
        int radius;
    public:
        CHtmlImageMapAreaCell( celltype t, wxString &coords, double pixel_scale = 1.0);
    ...
    DECLARE_NO_COPY_CLASS(CHtmlImageMapAreaCell)
};
...
CHtmlImageMapAreaCell::CHtmlImageMapAreaCell( CHtmlImageMapAreaCell::celltype t, wxString &incoords, double pixel_scale )
{
    int i;
...
CHtmlLinkInfo *CHtmlImageMapAreaCell::GetLink( int x, int y ) const
{
    switch (type)
    {
        case RECT:
    ...
    if (m_Next)
    {
        CHtmlImageMapAreaCell  *a = (CHtmlImageMapAreaCell*)m_Next;
...
//                  it is always placed before CHtmlImageMapAreaCells
//                  It responds to Find(wxHTML_COND_ISIMAGEMAP)
//--------------------------------------------------------------------------------


class CHtmlImageMapCell : public CHtmlCell
{
    public:
        CHtmlImageMapCell( wxString &name );
    protected:
        wxString m_Name;
    public:
        virtual CHtmlLinkInfo *GetLink( int x = 0, int y = 0 ) const;
        virtual const CHtmlCell *Find( int cond, const void *param ) const;
        void Draw(wxDC& WXUNUSED(dc),
                  int WXUNUSED(x), int WXUNUSED(y),
                  int WXUNUSED(view_y1), int WXUNUSED(view_y2),
                  CHtmlRenderingInfo& WXUNUSED(info)) {}

    DECLARE_NO_COPY_CLASS(CHtmlImageMapCell)
};


CHtmlImageMapCell::CHtmlImageMapCell( wxString &name )
{
    m_Name = name ;
}

CHtmlLinkInfo *CHtmlImageMapCell::GetLink( int x, int y ) const
{
    CHtmlImageMapAreaCell  *a = (CHtmlImageMapAreaCell*)m_Next;
    if (a)
        return a->GetLink( x, y );
    return CHtmlCell::GetLink( x, y );
}

const CHtmlCell *CHtmlImageMapCell::Find( int cond, const void *param ) const
{
    ...
    return CHtmlCell::Find(cond, param);
}
...
class CHtmlImageCell : public CHtmlCell
{
public:
    CHtmlImageCell(wxWindow *window,
...
#endif
    double              m_scale;
    CHtmlImageMapCell *m_imageMap;
    wxString            m_mapName;

    DECLARE_NO_COPY_CLASS(CHtmlImageCell)
};
...
{
    public:
        wxGIFTimer(CHtmlImageCell *cell) : m_cell(cell) {}
        virtual void Notify()
        {
...
CHtmlImageCell::CHtmlImageCell(wxWindow *window, wxFSFile *input,
                                 int w, int h, double scale, int align,
                                 const wxString& mapname) : CHtmlCell()
{
...
void CHtmlImageCell::SetImage(const wxImage& img)
{
    if ( img.Ok() )
    {
        delete m_bitmap;
...
#if wxUSE_GIF && wxUSE_TIMER
void CHtmlImageCell::AdvanceAnimation(wxTimer *timer)
{
    wxImage img;
    ...
    {
        m_physX = m_physY = 0;
        for (CHtmlCell *cell = this; cell; cell = cell->GetParent())
        {
...
void CHtmlImageCell::Layout(int w, int vw)
{
...
void CHtmlImageCell::Draw(wxDC& dc, int x, int y,
                           int WXUNUSED(view_y1), int WXUNUSED(view_y2),
                           CHtmlRenderingInfo& WXUNUSED(info))
{
    if ( m_showFrame )
...
CHtmlLinkInfo *CHtmlImageCell::GetLink( int x, int y ) const
{
    if (m_mapName.empty())
        return CHtmlCell::GetLink( x, y );
    if (!m_imageMap)
    {
        CHtmlContainerCell *p, *op;
        op = p = GetParent();
        while (p)
        {
            op = p;
        ...
        p = op;
        CHtmlCell *cell = (CHtmlCell*)p->Find(wxHTML_COND_ISIMAGEMAP,
        ...
        {   // dirty hack, ask Joel why he fills m_ImageMap in this place
            ...
            CHtmlImageMapCell **cx = (CHtmlImageMapCell**)(&m_imageMap);
            *cx = (CHtmlImageMapCell*)cell;
        }
    }
    ...
    TAG_HANDLER_PROC(tag)
    {
        if (tag.GetName() == wxT("IMG"))
        {
            if (tag.HasParam(wxT("SRC")))
            {
                ...
                CHtmlImageCell *cel = new CHtmlImageCell(
                                          m_WParser->GetWindow(),
                                          str, w, h,
        ...
        if (tag.GetName() == wxT("MAP"))
        {
            ...
            if (tag.HasParam(wxT("NAME")))
            {
                ...
                CHtmlImageMapCell *cel = new CHtmlImageMapCell( tmp );
                m_WParser->GetContainer()->InsertCell( cel );
            }
            ParseInner( tag );
            m_WParser->CloseContainer();
        ...
        if (tag.GetName() == wxT("AREA"))
        {
            if (tag.HasParam(wxT("SHAPE")))
            {
                ...
                tmp.MakeUpper();
                CHtmlImageMapAreaCell *cel = NULL;
                ...
                if (tmp == wxT("POLY"))
                {
                    cel = new CHtmlImageMapAreaCell( CHtmlImageMapAreaCell::POLY, coords, m_WParser->GetPixelScale() );
                }
                else if (tmp == wxT("CIRCLE"))
                {
                    cel = new CHtmlImageMapAreaCell( CHtmlImageMapAreaCell::CIRCLE, coords, m_WParser->GetPixelScale() );
                }
                else if (tmp == wxT("RECT"))
                {
                    cel = new CHtmlImageMapAreaCell( CHtmlImageMapAreaCell::RECT, coords, m_WParser->GetPixelScale() );
                }
                if (cel != NULL && tag.HasParam(wxT("HREF")))
                {
                    ...
                    cel->SetLink( CHtmlLinkInfo(tmp, target));
                }
