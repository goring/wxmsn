...
#pragma implementation "CWinPars.h"
#endif
...
IMPLEMENT_ABSTRACT_CLASS(CHtmlWinParser, CHtmlParser)

wxList CHtmlWinParser::m_Modules;

CHtmlWinParser::CHtmlWinParser(CHtmlWindow *wnd) : CHtmlParser()
{
    m_tmpStrBuf = NULL;
    m_tmpStrBufSize = 0;
    m_Window = wnd;
    ...
    // fill in CHtmlParser's tables:
    wxList::compatibility_iterator node = m_Modules.GetFirst();
    while (node)
    {
        CHtmlTagsModule *mod = (CHtmlTagsModule*) node->GetData();
        mod->FillHandlersTable(this);
        node = node->GetNext();
    }
}

CHtmlWinParser::~CHtmlWinParser()
{
    int i, j, k, l, m;
...
void CHtmlWinParser::AddModule(CHtmlTagsModule *module)
{
    m_Modules.Append(module);
}

void CHtmlWinParser::RemoveModule(CHtmlTagsModule *module)
{
...
void CHtmlWinParser::DoneParser()
{
    m_Container = NULL;
#if !wxUSE_UNICODE
    SetInputEncoding(wxFONTENCODING_ISO8859_1); // for next call
#endif
    CHtmlParser::DoneParser();
}

wxObject* CHtmlWinParser::GetProduct()
{
    CHtmlContainerCell *top;
...
wxFSFile *CHtmlWinParser::OpenURL(CHtmlURLType type,
                                   const wxString& url) const
{
    if ( m_Window )
    {
        wxString myurl(url);
        CHtmlOpeningStatus status;
        for (;;)
        {
    ...
    return CHtmlParser::OpenURL(type, url);
}
...
CHtmlContainerCell* CHtmlWinParser::OpenContainer()
{
    m_Container = new CHtmlContainerCell(m_Container);
...
CHtmlContainerCell* CHtmlWinParser::SetContainer(CHtmlContainerCell *c)
{
    m_tmpLastWasSpace = true;
        /* to avoid space being first character in paragraph */
...
CHtmlContainerCell* CHtmlWinParser::CloseContainer()
{
...
void CHtmlWinParser::SetFontSize(int s)
{
    if (s < 1) s = 1;
    else if (s > 7) s = 7;
    m_FontSize = s;
...
void CHtmlWinParser::SetLink(const CHtmlLinkInfo& link)
{
    m_Link = link;
...
void CHtmlWinParser::SetFontFace(const wxString& face)
{
...
#if !wxUSE_UNICODE
void CHtmlWinParser::SetInputEncoding(wxFontEncoding enc)
{
...
IMPLEMENT_ABSTRACT_CLASS(CHtmlWinTagHandler, CHtmlTagHandler)
...
IMPLEMENT_DYNAMIC_CLASS(CHtmlTagsModule, wxModule)

bool CHtmlTagsModule::OnInit()
{
    CHtmlWinParser::AddModule(this);
    return true;
}

void CHtmlTagsModule::OnExit()
{
    CHtmlWinParser::RemoveModule(this);
}
